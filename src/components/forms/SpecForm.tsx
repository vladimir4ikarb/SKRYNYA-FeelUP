...
export const SpecForm = ({ editingItem }: SpecFormProps) => {
  return (
    <div className="grid grid-cols-2 gap-8">
      <div>
        <label className="text-sm font-bold text-text-muted block mb-3">Розмір (напр. 12")</label>
        <input name="size" defaultValue={editingItem?.size} required className="input-field" />
      </div>
      <div>
        <label className="text-sm font-bold text-text-muted block mb-3">Об'єм гелію (л)</label>

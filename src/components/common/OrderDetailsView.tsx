...
import { Order, Purchase, OrderItem, PurchaseItem, Product, WriteOff, WriteOffItem } from '../../types';
...
interface OrderDetailsViewProps {
  ...
  onAddItem: () => void;
  onDeleteItem: (item: any, type: 'order' | 'purchase' | 'write-off') => void;
}
...
export const OrderDetailsView = ({
  ...
  if (!selectedOrderId && !selectedPurchaseId && !selectedWriteOffId) return null;
  ...
  const writeOff = selectedWriteOffId ? writeOffs.find(w => w.id === selectedWriteOffId) : null;
  
  const isLocked = selectedOrderId 
    ? order?.status === 'Виконано' || order?.status === 'Скасовано'
    : selectedPurchaseId
      ? purchase?.status === 'Проведено' || purchase?.status === 'Скасовано'
      : writeOff?.status === 'Проведено' || writeOff?.status === 'Скасовано';
  ...
  const items = selectedOrderId 
    ...
    : selectedPurchaseId
      ...
      : writeOffItems.filter(wi => wi.writeOffId === selectedWriteOffId);

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="card-base p-4 lg:p-8 mt-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <h3 className="text-lg lg:text-xl font-bold text-text-main flex items-center gap-2">
            {selectedOrderId ? 'Позиції замовлення' : selectedPurchaseId ? 'Позиції закупівлі' : 'Позиції списання'}
            {isLocked && <Clock className="w-5 h-5 text-amber-500" />}
          </h3>
          {selectedOrderId && (
            <button 
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-background/50">
              ...
              {(!selectedWriteOffId) && <th className="px-4 lg:px-8 py-4 text-[10px] lg:text-xs font-black text-text-muted uppercase tracking-widest leading-none text-right">Сума</th>}
          ...
          <tbody className="divide-y divide-border">
            {items.map(item => {
              ...
              const unitPrice = (item as any).costPrice ?? (item as any).price ?? 0;
              return (
                <tr key={item.id} className="hover:bg-background/30 transition-colors">
                  <td className="px-4 lg:px-8 py-4">
                    ...
                    {!selectedWriteOffId && <p className="text-[10px] text-text-muted mt-0.5">{unitPrice} ₴ / шт</p>}
                  </td>
                  <td className="px-4 lg:px-8 py-4 font-black text-text-muted text-sm">{item.qty}</td>
                  {!selectedWriteOffId && <td className="px-4 lg:px-8 py-4 font-black text-text-main text-sm lg:text-base text-right">{item.qty * unitPrice} ₴</td>}
                  <td className="px-4 lg:px-8 py-4 text-right">
                    {!isLocked && (
                      <button onClick={() => onDeleteItem(item, selectedOrderId ? 'order' : selectedPurchaseId ? 'purchase' : 'write-off')} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-500/10 rounded-lg transition-all">
